...
import com.kauailabs.nav6.QuaternionUpdate;
 ...
 * 
 * The IMUAdvanced class enables access to basic connectivity and state information, as well as key orientation information (yaw, pitch, roll, compass
 * heading). Additionally, the IMUAdvanced class also provides access to extended information including linear acceleration, motion detection, and
 * sensor temperature.
 * 
 * @author Scott
 */
public class IMUAdvanced extends IMU
{
	...
	private QuaternionUpdate quaternion_update_data;
	...
	/**
	 * Constructs the IMUAdvanced class, overriding the default update rate with a custom rate which may be from 4 to 100, representing the number of
	 * updates per second sent by the nav6 IMU.
	 * 
	 * Note that increasing the update rate may increase the CPU utilization. Note that calculation of some advanced values utilizes additional cpu
	...
	public IMUAdvanced(SerialPort serial_port, byte update_rate_hz)
	{
		...
		quaternion_update_data = new QuaternionUpdate();
	...
	/**
	 * Indicates if the nav6 IMU is currently detection motion, based upon the x and y-axis world linear acceleration values. If the sum of the
	...
	private void setQuaternion(QuaternionUpdate raw_update)
	{
		synchronized (this)
		{ // synchronized block
			...
			q_final[0] = q_product[0] * q_conjugate[0] - q_product[1] * q_conjugate[1] - q_product[2] * q_conjugate[2] - q_product[3]
					* q_conjugate[3];  // new w
			q_final[1] = q_product[0] * q_conjugate[1] + q_product[1] * q_conjugate[0] + q_product[2] * q_conjugate[3] - q_product[3]
					* q_conjugate[2];  // new x
			q_final[2] = q_product[0] * q_conjugate[2] - q_product[1] * q_conjugate[3] + q_product[2] * q_conjugate[0] + q_product[3]
					* q_conjugate[1];  // new y
			q_final[3] = q_product[0] * q_conjugate[3] + q_product[1] * q_conjugate[2] - q_product[2] * q_conjugate[1] + q_product[3]
					* q_conjugate[0];  // new z
