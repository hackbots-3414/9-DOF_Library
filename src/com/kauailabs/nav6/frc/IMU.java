 ...
 * 
 * The IMU class enables access to basic connectivity and state information, as well as key orientation information (yaw, pitch, roll, compass
 * heading).
 * 
...
public class IMU extends SensorBase implements PIDSource, LiveWindowSendable, Runnable
{
	...
	/**
	 * Constructs the IMU class, overriding the default update rate with a custom rate which may be from 4 to 100, representing the number of updates
	 * per second sent by the nav6 IMU.
	 * 
	 ...
	 * 
	 * Note that the returned yaw value will be offset by a user-specified offset value; this user-specified offset value is set by invoking the
	 * zeroYaw() method.
	 * 
	 ...
	 * 
	 * Note that this value is sensed by the nav6 magnetometer, which can be affected by nearby magnetic fields (e.g., the magnetic fields generated
	 * by nearby motors).
	 * 
	...
	/**
	 * Indicates whether the nav6 IMU is currently connected to the host computer. A connection is considered established whenever a value update
	...
	/**
	 * Returns the count in bytes of data received from the nav6 IMU. This could can be useful for diagnosing connectivity issues.
	 * 
	...
	/**
	 * Returns the count of valid update packets which have been received from the nav6 IMU. This count should increase at the same rate indicated by
	...
	/**
	 * Returns true if the nav6 IMU is currently performing automatic calibration. Automatic calibration occurs when the nav6 IMU is initially powered
	 * on, during which time the nav6 IMU should be held still.
	 * 
	 * During this automatically calibration, the yaw, pitch and roll values returned may not be accurate.
	 * 
	 * Once complete, the nav6 IMU will automatically remove an internal yaw offset value from all reported values.
	 * 
	...
	/**
	 * Returns the current yaw value reported by the nav6 IMU. This yaw value is useful for implementing features including "auto rotate to a known
	 * angle".
	 * 
